...
def run(
    ...
    # --- PATCH dla torch>=2.6 (globalne wyłączenie weights_only) ---
    import torch.serialization

    _orig_serialization_load = torch.serialization.load
    def _unrestricted_serialization_load(*args, **kwargs):
        kwargs.setdefault("weights_only", False)
        return _orig_serialization_load(*args, **kwargs)
    torch.serialization.load = _unrestricted_serialization_load
    torch.load = _unrestricted_serialization_load

    # Dodatkowo whitelist YOLOv5 klasy
    try:
        import importlib
        models_yolo = importlib.import_module("models.yolo")
        torch.serialization.add_safe_globals([models_yolo.Model])
    except Exception as e:
        print("⚠️ Nie udało się dodać safe_globals:", e)
    # ---------------------------------------------------------------
